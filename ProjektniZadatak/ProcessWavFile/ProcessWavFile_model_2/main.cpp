...
#define INPUT_GAIN (FRACT_NUM(pow(10.0, input_gain / 20)))
#define HEADROOM_GAIN (FRACT_NUM(pow(10.0, headroom_gain / 20)))
#define OUTPUT_GAIN (FRACT_NUM(pow(10.0, output_gain / 20)))

fract sampleBuffer[MAX_NUM_CHANNEL][BLOCK_SIZE];
fract tremoloBuffer[TREM_BUFF_CHANNELS][BLOCK_SIZE];
...
void processing()
{
	fract headroom;
	resetIterators();
	fract* tremoloLeftIterator = tremoloBuffer[L_TREM];
	fract* tremoloRightIterator = tremoloBuffer[R_TREM];
	...
	for (int i = 0; i < BLOCK_SIZE; i++)
	{
		if (selected_mode != MODE_0_2_0)
		{
			...
			headroom = headroom * (fract)HEADROOM_GAIN;
...
// input.wav output.wav enable input_gain headroom_gain output_mode
	...
	if (argc > 4)
	{
		...
		if (input_gain > FRACT_NUM(0.0))
		{
			printf("Input gain too large, using default (-6 dB)\n");
			input_gain = FRACT_NUM(-6.0);
		}
	}
	if (argc > 5)
	{
	...
	//-------------------------------------------------	
	{
		...
		{	
			...
			for(int j=0; j<BLOCK_SIZE; j++)
			{
				...
				{	
					sample = sampleBuffer[k+offset][j].toLong();	// crude, non-rounding 			
